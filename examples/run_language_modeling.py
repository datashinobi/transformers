...
def train(args, train_dataset, model: PreTrainedModel, tokenizer: PreTrainedTokenizer) -> Tuple[int, float]:
    ...
    # multi-gpu training (should be after apex fp16 initialization) add invariant check for multi-gpu single node training
    if args.n_gpu > 1 and args.n_gpu == args.world_size: 
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                model.zero_grad()
                global_step += 1
                if  torch.distributed.get_rank() == 0: # writes to tb and checkpoints saving on rank 0
